...
export async function syncAssets(req, res) {
    try {
        ...
        await Assets.bulkWrite(operations);

        res.status(200).json({
            message: `Synced ${assets.length} assets successfully`,
            count: assets.length
        });
    } catch (error) {
...
export async function getModifiedAssets(req, res) {
    try {
        const { lastSyncTimestamp } = req.query;

        if (!lastSyncTimestamp) {
            return res.status(200).json(await Assets.find());
        }

        const lastSyncDate = new Date(parseInt(lastSyncTimestamp));

        const modifiedAssets = await Assets.find({
            lastModified: { $gte: lastSyncDate }
        });

        res.status(200).json(modifiedAssets);
    } catch (error) {
        console.error('Error getting modified assets:', error);
        res.status(500).json({ message: 'Server error' });
    }
}
